...
public final class AsyncapiProxyFactory implements AsyncapiStreamFactory
{
    ...
    final class AsyncapiClientStream
    {
            ...
            long traceId)
        {
            if (!BindingHandlerState.replyClosed(state))
            {
                doReset(asyncapi, state, originId, routedId, replyId, replySeq, replyAck, replyMax,
                    traceId, authorization, EMPTY_OCTETS);
            ...
            int padding)
        {
            ...
            doWindow(asyncapi, state, originId, routedId, replyId, replySeq, replyAck, replyMax,
                traceId, authorization, budgetId, padding + replyPad);
        }
        ...
        private void doAsyncapiClientData(
            ...
            Flyweight extension)
        {
            doData(asyncapi, state, originId, routedId, initialId, initialSeq, initialAck, initialMax,
                traceId, authorization, budgetId, flags, reserved, payload, extension);
        ...
        private void doAsyncapiClientFlush(
            ...
            OctetsFW extension)
        {
            doFlush(asyncapi, state, originId, routedId, initialId, initialSeq, initialAck, initialMax,
                traceId, authorization, initialBud, reserved, extension);
        }
            ...
            OctetsFW extension)
        {
            if (!BindingHandlerState.initialClosed(state))
            {
                doEnd(asyncapi, state, originId, routedId, initialId, initialSeq, initialAck, initialMax,
                    traceId, authorization, extension);
            ...
            OctetsFW extension)
        {
            if (!BindingHandlerState.initialClosed(state))
            {
                doAbort(asyncapi, state, originId, routedId, initialId, initialSeq, initialAck, initialMax,
                    traceId, authorization, extension);
