...
public class KafkaDescribeConfigsNetworkIT
{
    private final K3poRule k3po = new K3poRule()
        .addScriptRoot("net", "io/aklivity/zilla/runtime/command/dump/binding/kafka/streams/network/describe.configs.v0");

    private final TestRule timeout = new DisableOnDebug(new Timeout(5, SECONDS));

    private final DumpRule dump = new DumpRule()
        .labels("test", "net0", "kafka", "server")
        .bindings("test.net0", "test.kafka", "test.server", "test.0", "test.kafka");
