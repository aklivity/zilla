...
public class MqttKafkaSessionFactory implements MqttKafkaStreamFactory
{
        ...
        IntPropertyDef reconnectDelay)
    {
        ...
        this.handleReconnectWillStream = this::onKafkaSignalConnectWillStream;
    }
        ...
        long bindingId)
    {
        ...
        if (willAvailable && coreIndex == 0)
        {
            ...
            this.reconnectAt = signaler.signalAt(
                ...
                this::onKafkaSignalConnectWillStream);
        }
        sessionIds.put(bindingId, supplySessionId.get());
    }

    private void onKafkaSignalConnectWillStream(
        int signalId)
    {
    ...
    public final class KafkaWillProxy
    {
            ...
            long affinity)
        {
            ...
            kafka = newWillStream(this::onWillMessage, originId, routedId, initialId, 0, 0, 0,
                ...
                if (key != null)
                {
                    if (payload == null)
                    {
                        final String clientId0 = key.value()
                            .get((b, o, m) -> b.getStringWithoutLengthUtf8(o, m - o - WILL_SIGNAL_KEY_POSTFIX.length()));
                    ...
                    if (willSignal != null)
                    {
                        ...
                        willFetchers.put(clientId, willFetcher);
                    }
                }
            }
        }
    ...
    private final class KafkaFetchWillProxy
    {
        ...
        private void releaseBufferSlot(
            long traceId,
            long authorization)
        {
    ...
    private final class KafkaSessionSignalProxy extends KafkaSessionProxy
    {
        ...
        protected void doKafkaBegin(long traceId, long authorization, long affinity)
        {
            ...
            kafka = newKafkaStream(super::onKafkaMessage, originId, routedId, initialId, initialSeq, initialAck, initialMax,
                ...
                delegate.sessionId, KafkaCapabilities.PRODUCE_AND_FETCH);
        }
    ...
    private final class KafkaSessionStateProxy extends KafkaSessionProxy
    {
            ...
            long affinity)
        {
            ...
            kafka = newKafkaStream(super::onKafkaMessage, originId, routedId, initialId, initialSeq, initialAck, initialMax,
                ...
                delegate.sessionId, capabilities);
        }
            ...
            WindowFW window)
        {
            ...
            if (!wasOpen && !isSetCleanStart(delegate.sessionFlags))
            {
