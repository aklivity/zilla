...
import io.aklivity.zilla.engine.drive.internal.Info;
import io.aklivity.zilla.engine.drive.internal.LabelManager;
import io.aklivity.zilla.engine.drive.internal.Tuning;
import io.aklivity.zilla.engine.drive.internal.context.ConfigureTask;
import io.aklivity.zilla.engine.drive.internal.context.DispatchAgent;
import io.aklivity.zilla.engine.drive.internal.stream.NamespacedId;

public final class Drive implements AutoCloseable
{
    ...
    private final List<DriveExtSpi> extensions;
    private final DriveExtContext context;
    ...
    Drive(
        ...
        URL configURL,
        int coreCount,
        Collection<DriveAffinity> affinities)
    {
        ...
        tuning.reset();
        for (DriveAffinity affinity : affinities)
        {
        ...
        final List<DriveExtSpi> extensions = ServiceLoader.load(DriveExtSpi.class).stream()
                .map(Provider::get)
                .collect(toList());
    ...
    private final class ContextImpl implements DriveExtContext
    {
        ...
        private final class LoadImpl implements DriveStats
        {
