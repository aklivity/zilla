...
final class McpProxyResourcesSubscribeFactory extends McpProxyItemFactory
{
    McpProxyResourcesSubscribeFactory(
        McpConfiguration config,
        EngineContext context,
        LongFunction<McpBindingConfig> supplyBinding)
    {
        super(config, context, supplyBinding, McpBeginExFW.KIND_RESOURCES_SUBSCRIBE);
    }
    ...
    {
        builder.resourcesSubscribe(r -> r.sessionId(sessionId).uri(upstream.resourcesSubscribe().uri().asString()));
    }
    ...
    {
        return beginEx.resourcesSubscribe().sessionId().asString();
    }
