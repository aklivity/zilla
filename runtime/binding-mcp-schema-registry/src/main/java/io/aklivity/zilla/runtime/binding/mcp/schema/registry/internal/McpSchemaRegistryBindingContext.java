...
{
    private final Map<KindConfig, McpSchemaRegistryProxyFactory> factories;

    McpSchemaRegistryBindingContext(
        McpSchemaRegistryConfiguration config,
        EngineContext context)
    {
        Map<KindConfig, McpSchemaRegistryProxyFactory> factories = new EnumMap<>(KindConfig.class);
        factories.put(PROXY, new McpSchemaRegistryProxyFactory(config, context));
    ...
    {
        McpSchemaRegistryProxyFactory factory = factories.get(binding.kind);
