...
/**
 * The result of a single {@link AvroPipeline#transform} call: the {@link Status} plus the source bytes
 * {@link #consumed()} from the input window and the output bytes {@link #produced()} into the destination
 * buffer. Reused across every {@code transform} call of a single {@link AvroPipeline}; read the fields
 * immediately and do not retain the instance beyond the next call.
 */
public final class AvroPipelineResult
{
     ...
     * {@link AvroPipeline#remaining()} on {@link Status#STARVED} or {@link Status#COMPLETED}.
     */
    public int consumed()
    {
        return consumed;
