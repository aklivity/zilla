...
public class StringModelTest
{
    ...
    {
        StringModel model = new StringModel(List.of());

        assertEquals(StringModel.NAME, model.name());
    }

    @Test
    public void shouldSupplyContextForEachInstalledExtension()
    {
        StringModelExtContext extContext = config -> null;
        ...
        StringModel model = new StringModel(List.of(ext));
        ModelContext context = model.supply(mock(EngineContext.class));

        assertThat(context, instanceOf(StringModelContext.class));
    }
}
