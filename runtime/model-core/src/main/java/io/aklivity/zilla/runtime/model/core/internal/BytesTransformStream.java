...
// BytesExtModelPipeline to bind sink-to-sink. A stage holds the in-flight state of one value, so a fresh
// stream is folded per pipeline rather than once per model configuration.
final class BytesTransformStream implements BytesTransformable<BytesTransformStream>
{
    private final List<BytesTransform> transforms;

    BytesTransformStream()
    {
        this.transforms = new ArrayList<>();
    }

    @Override
    public BytesTransformStream transform(
        BytesTransform transform)
    {
        ...
        if (transform != BytesTransform.NONE)
        {
