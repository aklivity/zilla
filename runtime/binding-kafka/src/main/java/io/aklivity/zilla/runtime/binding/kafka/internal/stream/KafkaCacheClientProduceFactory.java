            ...
            if ((flags & FLAGS_INIT) != 0x00)
            {
                ...
                if (stream.segment != null)
                {
                    ...
                    if (partition.writeProduceEntryStart(context, routedId, partitionOffset, stream.segment,
                        stream.entryMark, stream.valueMark, stream.valueLimit, timestamp, stream.initialId, producerId,
                        producerEpoch, sequence, ackMode, key, keyHash, valueLength, headers, trailersSizeMax,
                        valueFragment, convertKey, convertValue) == -1)
                    {
