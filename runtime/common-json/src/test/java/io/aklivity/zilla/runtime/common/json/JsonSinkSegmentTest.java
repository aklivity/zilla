...
class JsonSinkSegmentTest
{
    ...
    void shouldCompleteAcrossFrames()
    {
        ...
        pipeline.reset();
        assertEquals(Status.STARVED, pipeline.feed(new UnsafeBufferEx(first), 0, first.length, false));
        Status status = pipeline.feed(new UnsafeBufferEx(second), 0, second.length);
