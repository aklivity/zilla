...
public interface JsonSchema
{
     ...
     * stage's own {@link JsonEventConsumer.Status} reflects the schema verdict — {@link
     * JsonEventConsumer.Status#COMPLETE} when the value validates, {@link
     * JsonEventConsumer.Status#REJECTED} when it does not. Rejection is reported at the value
     * boundary, after the forwarded events have already been emitted, so callers abort the output
     * stream on {@code REJECTED} (emit-then-abort).
     */
    JsonEventConsumer validator(
