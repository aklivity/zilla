...
## Q3 — bounded output (the general case; validate is not special)

Output is **always** drained through the bounded sink — generated bytes and copied verbatim
bytes alike. Validate is just the transform whose run happens to be identity-sized; the pipeline
must not lean on that. So model verbatim as "copy a resident span **into the bounded sink**,"
which backpressures exactly like rendering a large generated value does today.

Three forward-only positions, none of which require backing up the input:

1. **validation cursor** — advances token-by-token as the parser validates; never rewinds.
2. **aggregate output cursor** — how much of the current run/value has been written to the sink;
   held in the sink's `resume()` state, advanced across `SUSPENDED` cycles. This is **run-level**,
   and is the position `consumed()` cannot express: `consumed()`/`remaining()` are *input-side,
   per-token* (validator→parser), whereas this is *output-side, per-aggregate* (sink→downstream).
3. **buffer residency** — the input window is held for the whole `feed` call, including every
   `SUSPENDED` cycle, so a run's source bytes remain available to resume from.

Flow: building a run is **span-extension only** (advancing a `[runStart, cursor)` reference over
the resident input — no copy, so no need to *anticipate* sink space). At flush the sink copies as
much of the run as fits, returns `SUSPENDED` with the aggregate output cursor parked; the caller
drains, re-targets the output, and `resume()` continues copying from that cursor. A `VERBATIM`
run is therefore delivered in output-sized pieces (`deferredBytes()` until the last), the same
cadence `SEGMENT` already uses. Because building copies nothing, capping the run to sink space up
front would only fragment runs for no gain — optimistic build + partial drain is correct.

Key fact that keeps this rewind-free: **validation-consumed != buffer-released**. A verbatim copy
is a `memcpy` over resident bytes, decoupled from parse state, so resuming a partially-drained run
re-reads resident input — no parser rewind. Runs are bounded by `min(window residency, …)`: flush
the pending span at the feed-window boundary before the window is released; we do not coalesce
across windows.

(An unbounded downstream could let the sink reference the input span zero-copy instead of copying
into its buffer — a possible optimization, not the model. The model is bounded-sink drain.)
