...
import static io.aklivity.zilla.config.binding.kafka.grpc.internal.KafkaGrpcOptionsConfigAdapter.DEFAULT;
...
public final class KafkaGrpcBindingConfig
{
        ...
        MutableDirectBufferEx metaBuffer)
    {
        ...
        this.routes = binding.routes.stream().map(r -> new KafkaGrpcRouteConfig(options, r))
            .collect(toList());
        this.helper = new KafkaGrpcFetchHeaderHelper(options.correlation, metaBuffer);
    }
}
