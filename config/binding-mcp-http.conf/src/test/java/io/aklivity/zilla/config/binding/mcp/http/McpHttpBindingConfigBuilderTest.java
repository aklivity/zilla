/*
 * Copyright 2021-2026 Aklivity Inc
 *
 * Licensed under the Aklivity Community License (the "License"); you may not use
 * this file except in compliance with the License.  You may obtain a copy of the
 * License at
 *
 *   https://www.aklivity.io/aklivity-community-license/
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
 * WARRANTIES OF ANY KIND, either express or implied.  See the License for the
 * specific language governing permissions and limitations under the License.
 */
package io.aklivity.zilla.config.binding.mcp.http;

import static io.aklivity.zilla.config.engine.KindConfig.PROXY;
import static org.hamcrest.MatcherAssert.assertThat;
import static org.hamcrest.Matchers.equalTo;
import static org.hamcrest.Matchers.hasSize;
import static org.hamcrest.Matchers.instanceOf;

import org.junit.Test;

import io.aklivity.zilla.config.binding.mcp.http.internal.McpHttpBindingInfo;
import io.aklivity.zilla.config.engine.ConditionConfig;
import io.aklivity.zilla.config.engine.RouteConfig;
import io.aklivity.zilla.config.engine.WithConfig;

public class McpHttpBindingConfigBuilderTest
{
    @Test
    public void shouldBuildBindingViaFluentKindSpecificChain()
    {
        McpHttpBindingConfig binding = McpHttpBindingConfig.builder()
            .namespace("test")
            .name("mcp_http0")
            .kind(PROXY)
            .options()
                .tool(McpHttpToolConfig.builder()
                    .name("test")
                    .build())
                .build()
            .route()
                .when()
                    .tool("test")
                    .build()
                .with()
                    .header("test", "value")
                    .build()
                .build()
            .build();

        assertThat(binding.namespace, equalTo("test"));
        assertThat(binding.name, equalTo("mcp_http0"));
        assertThat(binding.type, equalTo(McpHttpBindingInfo.TYPE));
        assertThat(binding.kind, equalTo(PROXY));

        McpHttpOptionsConfig options = (McpHttpOptionsConfig) binding.options;
        assertThat(options.tools, hasSize(1));
        assertThat(options.tools.get(0).name, equalTo("test"));

        assertThat(binding.routes, hasSize(1));

        RouteConfig route = binding.routes.get(0);
        assertThat(route.when, hasSize(1));

        ConditionConfig condition = route.when.get(0);
        assertThat(condition, instanceOf(McpHttpConditionConfig.class));
        assertThat(((McpHttpConditionConfig) condition).tool, equalTo("test"));

        assertThat(route.with, instanceOf(McpHttpWithConfig.class));
        WithConfig with = route.with;
        assertThat(((McpHttpWithConfig) with).headers.get("test"), equalTo("value"));
    }
}
