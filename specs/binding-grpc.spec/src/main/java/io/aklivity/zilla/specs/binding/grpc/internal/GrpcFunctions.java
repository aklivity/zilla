...
public final class GrpcFunctions
{
    ...
    public static final class GrpcBeginExBuilder
    {
        ...
        private GrpcBeginExBuilder metadata(
            ...
            String value)
        {
            DirectBufferEx nameBuffer = new UnsafeBufferEx(name.getBytes());
            OctetsFW nameOctets = new OctetsFW().wrap(nameBuffer, 0, nameBuffer.capacity());
            DirectBufferEx valueBuffer = new UnsafeBufferEx(value.getBytes());
    ...
    public static final class GrpcMessageBuilder
    {
        ...
        private final MutableDirectBufferEx messageBuffer = new UnsafeBufferEx(new byte[1024 * 200]);

        private int messageBufferLimit = 5;

        private GrpcMessageBuilder()
        {
            MutableDirectBufferEx keyBuffer = new UnsafeBufferEx(new byte[1024 * 8]);
            keyRW = new Varuint32FW.Builder().wrap(keyBuffer, 0, keyBuffer.capacity());
            MutableDirectBufferEx lenBuffer = new UnsafeBufferEx(new byte[1024 * 8]);
