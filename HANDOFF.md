...
| 3 — `with.cache` static credential over `options.cache.authorization` | **DONE+pushed.** Per-route `with.cache.credentials` added to `McpWithConfig` (+`McpWithCacheConfig`/builder), `McpWithConfigAdapter`, and the route `with` schema block. Carried on `McpRoutePrefix` (3rd component, null-default 2-arg ctor kept for the live path) via `McpBindingConfig.resolveAll(kind,auth)`. Hydrater (`startRoutes`) reauthorizes the per-route credential through the **cache guard** to a per-route authorization for that route's list stream, falling back to `cache.authorization` (= `options.cache.authorization`) when absent → precedence `with` > `options`. Lifecycle stream still uses the binding cache authorization (list-only, per Phase 3 scope). New `McpWithConfigAdapterTest` (4) + scenario `cache.hydrate.credentials.toolkit` (server+client, parameterized `listAuthorization` property; k3po routes server accepts by `(bindingId,authorization)` so lifecycle@0 and list@1 coexist on one exit address) wired into `McpProxyCacheIT#shouldHydrateToolkitWithRouteCredentials` (server-only, hydrate filter=tools) and peer-to-peer `ProxyCacheIT#shouldHydrateToolkitWithRouteCredentials`. Negative-checked (reverting wiring → IT times out). Full no-skip green: runtime 200 ITs/UT + jacoco/checkstyle/license/notice; spec 185 + gates. |
...
- **3**: **DONE** (see status table). Shipped `with.cache.credentials` per-route
  static credential with precedence over `options.cache.authorization`,
  list-only (per-route list hydration stream); lifecycle still uses the binding
  cache authorization. Note for Phase 5/6: if per-identity / per-route lifecycle
  credentials are needed, the lifecycle fan-out (`McpLifecycleClient` uses one
  `server.authorization` for all routes) must be extended — out of Phase 3 scope.
