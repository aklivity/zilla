...
public final class BoundedOctets32FlyweightGenerator extends ClassSpecGenerator
{
    private final TypeSpec.Builder classBuilder;
    private final ClassName visitorClass;
    private final BuilderClassBuilder builderClassBuilder;

    public BoundedOctets32FlyweightGenerator(
        ClassName flyweightType,
        ClassName boundedOctetsType)
    {
        super(flyweightType.peerClass("BoundedOctets32FW"));
    ...
    private MethodSpec toStringMethod()
    {
        return methodBuilder("toString")
            ...
            .addStatement("return String.format(\"boundedOctets32[%d]\", length())")
            .build();
    }
    ...
    {
        private final TypeSpec.Builder classBuilder;
        private final ClassName boundedOctets32BuilderType;
        private final ClassName boundedOctetsType;

        private BuilderClassBuilder(
            ClassName boundedOctets32Type,
            ClassName boundedOctetsType)
        {
            this.boundedOctets32BuilderType = boundedOctets32Type.nestedClass("Builder");
            this.boundedOctetsType = boundedOctetsType;
            ClassName boundedOctetsBuilderType = boundedOctetsType.nestedClass("Builder");
            this.classBuilder = classBuilder(boundedOctets32BuilderType.simpleName())
                .addModifiers(PUBLIC, STATIC, FINAL)
                .superclass(ParameterizedTypeName.get(boundedOctetsBuilderType, boundedOctets32Type));
        }
        ...
        private MethodSpec setWithFlyweight()
        {
            return methodBuilder("set")
                .addAnnotation(Override.class)
                .addModifiers(PUBLIC)
                .returns(boundedOctets32BuilderType)
                .addParameter(boundedOctetsType, "value")
                .addStatement("int newLimit = offset() + LENGTH_SIZE + value.length()")
                .addStatement("checkLimit(newLimit, maxLimit())")
                .addStatement("buffer().putInt(offset() + LENGTH_OFFSET, value.length(), byteOrder)")
